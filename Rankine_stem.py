...
class rankine():
    ...
    def calc_efficiency(self):
        ...
        if self.t_high is None:
            self.state1 = steam(pressure=self.p_high, x=1, name='Turbine Inlet')
        else:
            self.state1 = steam(pressure=self.p_high, T=self.t_high, name='Turbine Inlet')
        ...
        # state 4: pump exit (p_high,s=s_pump_inlet) typically subcooled, but estimate as saturated liquid
        self.state4 = steam(pressure=self.p_high, x=0, name='Pump Exit')
...
def main():
    rankine1 = rankine(p_high=8000, p_low=8, t_high=None, name='Rankine Cycle 1')
