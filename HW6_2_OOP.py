...
class Node():
    ...
    def getNetFlowRate(self):
        ...
        '''
        Qtot = self.extFlow  # Include external flow
        for p in self.pipes:
...
class Pipe():
    ...
    def FrictionFactor(self):
        ...
        if Re >= 4000:  # true for turbulent flow
            ...
            return result[0]
        elif Re <= 2000:  # true for laminar flow
            return lam()
        else:  # transition flow is ambiguous, so use normal variate weighted by Re
            CBff = CB(0.01)
            Lamff = lam()
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        hl = ff * (self.length / self.d) * ((self.fluid.rho * self.vel ** 2) / 2)
        return hl
...
def main():
    ...
    # Add Pipe objects to the PipeNetwork
    PN.pipes.append(Pipe('a', 'b', 250, 300, 0.00025, water))
    PN.pipes.append(Pipe('a', 'c', 100, 200, 0.00025, water))
    PN.pipes.append(Pipe('b', 'e', 100, 200, 0.00025, water))
    PN.pipes.append(Pipe('c', 'd', 125, 200, 0.00025, water))
    PN.pipes.append(Pipe('c', 'f', 100, 150, 0.00025, water))
    PN.pipes.append(Pipe('d', 'e', 125, 200, 0.00025, water))
    PN.pipes.append(Pipe('d', 'g', 100, 150, 0.00025, water))
    PN.pipes.append(Pipe('e', 'h', 100, 150, 0.00025, water))
    PN.pipes.append(Pipe('f', 'g', 125, 250, 0.00025, water))
    PN.pipes.append(Pipe('g', 'h', 125, 250, 0.00025, water))

    # Add Loop objects to the PipeNetwork
    ...
    if flow_rates is not None:
        ...
        print("Flow rate calculation failed.")
